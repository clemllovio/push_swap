...
/*   Updated: 2023/03/09 15:23:09 by cllovio          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../header/push_swap.h"

static int	is_valid(t_parsing *parsing)
{
	int	i;
	int	nbr_int;
...
static int	check_minus(t_parsing *parsing)
{
	int	i;
...
static int	check_dupe(t_parsing *parsing)
{
	int	i;
	int	j;
...
static int	create_tab_int(t_parsing *parsing)
{	
	int	i;

	i = 0;
	parsing->tab_int = malloc(sizeof(int) * (parsing->nbr_line_tab * 2));
	if (!(parsing->tab_int))
		return (-1);
	while (i < parsing->nbr_line_tab)
	{
